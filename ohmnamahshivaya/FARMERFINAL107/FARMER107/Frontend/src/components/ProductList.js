...
const ProductList = () => {
  ...
  const fetchProducts = async () => {
    ...
    const response = await fetch("https://finalproject-2-mdww.onrender.com/api/products", {
      headers: {
  ...
  const handleDelete = async (id) => {
    const token = localStorage.getItem("token");
    const response = await fetch(`https://finalproject-2-mdww.onrender.com/api/products/${id}`, {
      method: "DELETE",
