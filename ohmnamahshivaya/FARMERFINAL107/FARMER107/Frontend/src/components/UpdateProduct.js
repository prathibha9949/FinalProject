...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await axios.get(`https://finalproject-2-mdww.onrender.com/api/products/${id}`);
        if (res.data && res.data.equipment) {
          ...
          if (photo) {
            setPreviewImage(`https://finalproject-2-mdww.onrender.com${photo}`);
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://finalproject-2-mdww.onrender.com/api/products/${id}`, formDataToSend, {
